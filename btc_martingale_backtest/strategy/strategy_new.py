...
class NewModifiedMartingaleStrategy(bt.Strategy):
    """
    물타기 전략
    inputTrade, dividedLongCount, leverage 조정
    하락장은 기가 막히게 잘 피하는 전략이다. 다만 그러다보니 진입 기회가 적어서 수익률을 높이기 위한 변경.
    ...
    params = dict(
        inputTrade=10,
        profit=1.004,  # 0.4%에서 1.5%로 상향 조정
    ...
    def next(self):
        ...
        entry_condition = (
            ...
            close > open and
            abs(var) <= self.p.max_var and
            rf_pred >= self.p.rf_threshold
        )
        ...
        if self.entryCount >= 1 and self.entryCount < self.p.inputTrade:
            ...
            if price_gap > stoploss * self.entryCount and rf_pred >= self.p.rf_threshold_partial :
                ...
                self.save_trade_log('martingale', 
                                    ...
                                    entry_count=self.entryCount)
        # 부분 청산
        if self.entryCount >= 2 and (close > self.avgPrice * self.p.partial_exit_profit) :
        ...
        # 전체 청산
        if self.entryCount == 1 and (close >= self.avgPrice * self.p.profit):
            self.close()
